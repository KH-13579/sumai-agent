...
| モデル | 目安応答時間（CPU） | 備考 |
|---|---|---|
| `qwen2.5:3b` | 15〜40秒 | 速度優先。GPU なし環境に最適（**既定・チーム統一**） |
| `qwen2.5:7b` | 30〜90秒 | バランス型 |
...
5. 続けて **メーカー推薦AI** が要件・間取りをもとに最適なハウスメーカー・ポータルを最大3件推薦
6. 画面下部の送客バナーから公式サイトへ遷移、または「印刷 / PDFで持ち出す」で商談用に出力
...
要件（家族構成・予算・土地の有無）と生成した間取りをもとに、ハウスメーカー／情報ポータルを
最大3件、推薦理由・価格帯・注意点つきで提示する。UI では推薦カードと送客バナーに表示される。
...
現在のフローは ① ヒアリング → ② 間取り生成 → ③ 法規チェック → ④ メーカー推薦 → ⑤ 送客。
以降が未実装の項目。

- ⑥ 概算見積AI（坪単価ベース）
- ⑦ 来場予約連携（外部サイト）
- e-Gov 法令API による条文 RAG（`LegalCheckOutput.references` に受け口あり）
- 不動産情報ライブラリ API 連携（`REINFOLIB_API_KEY` 設定時のみ照会。エンドポイントは要スパイク）
