...
ハウスメーカー側システムや下流エージェント（見積AI・メーカー推薦AI）が
そのまま読み込める形で提供する。
...
class SumaiArtifact(BaseModel):
    ...
    # ── Phase 2（スロットのみ先に定義。下流エージェント実装時に型を差し替える）──
    estimate: Optional[dict[str, Any]] = Field(None, description="概算見積（見積AI／Phase 2）")
    maker_recommendation: Optional[dict[str, Any]] = Field(None, description="メーカー推薦（メーカー推薦AI／Phase 2）")
